...
test("an event whose ONLY tag is hallucinated is dropped, same as a sentence with no gameplay tag at all", async () => {
  await withFoundryGlobals(async () => {
    const api = new GrandDesignApi();
    const actor = createMockActor();
    api.setProposalAdapter(async () => ({
...
test("a proposal with a fundamentally wrong shape still fails the whole call loudly, unchanged", async () => {
  // Unlike events, a malformed PROPOSAL is still a hard failure -- see api.test.mjs's existing
  // coverage of this ({kind, skillEntry} instead of {kind, entry}). This test only confirms the
  // event-tolerance changes above did not accidentally soften that, since both paths now share the
  // same analyzeSessionNotes function.
  await withFoundryGlobals(async () => {
    const api = new GrandDesignApi();
    const actor = createMockActor();
    api.setProposalAdapter(async () => ({
    ...
    await assert.rejects(() => api.analyzeSessionNotes(actor, NOTES), /Invalid AI skill proposal/);
  });
});
